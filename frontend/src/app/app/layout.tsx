...
export default function AppWorkspaceLayout({ children }: { children: React.ReactNode }) {
  ...
  const getActiveTab = () => {
    ...
    if (pathname.includes("/donation-centers") || pathname.includes("/donation")) return "donation";
  ...
  if (!user) {
    return null; // AuthProvider's Route Guard redirects unauthenticated visitors to "/"
  }
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex bg-background text-foreground font-sans relative selection:bg-primary/20 selection:text-primary">
      ...
      {/* 2. Main Enterprise Workspace (Sticky Header + Independent Scroll Viewport) */}
      <div
        className={`flex-1 flex flex-col h-screen min-w-0 overflow-hidden transition-all duration-300 ${isCollapsed ? "lg:pl-20" : "lg:pl-68"
          }`}
      >
        {/* Sticky Top Header */}
        <AppHeader
